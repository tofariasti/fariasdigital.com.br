...
function SocialIcon({ children }: { children: React.ReactNode }) {
  return <span className="footer-social__icon">{children}</span>
}
  ...
  return (
    <footer className="site-footer">
      <section className="footer-cta" aria-labelledby="footer-cta-heading">
        <div className="container footer-cta__grid">
          <AnimatedSection>
            <h2 id="footer-cta-heading" className="footer-cta__title">{t(uiCopy.footerCta.title)}</h2>
          </AnimatedSection>
          <AnimatedSection delay={1}>
            <p className="footer-cta__text">{t(uiCopy.footerCta.text)}</p>
            <div className="footer-cta__actions">
              <WhatsAppButton waKey="geral" className="btn btn--primary">
                {t(uiCopy.footerCta.getInTouch)}
          ...
          <AnimatedSection delay={2}>
            <a
              ...
              aria-label={t(uiCopy.footerCta.chatWhatsApp)}
            >
              <svg className="footer-cta__whatsapp-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              ...
              <span className="footer-cta__whatsapp-label" aria-hidden="true">{t(uiCopy.footerCta.chatWhatsApp)}</span>
            </a>
          </AnimatedSection>
        </div>
      </section>
      <div className="container">
        <div className="footer-tech" aria-label={t(uiCopy.footer.techStack)}>
          {TECH_STACK.map((tech) => (
            <a
              key={tech.name}
              href={tech.href}
        ...
        <div className="footer-grid">
          <AnimatedSection>
            ...
            <div className="footer-social" aria-label={t(uiCopy.footer.social)}>
              <a href={config.links.linkedin} target="_blank" rel="noopener noreferrer" className="footer-social__link">
                <SocialIcon>
              ...
              <a href={config.links.github} target="_blank" rel="noopener noreferrer" className="footer-social__link">
                <SocialIcon>
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              ...
              <a href={config.links.instagramDrone} target="_blank" rel="noopener noreferrer" className="footer-social__link">
                <SocialIcon>
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <AnimatedSection delay={1}>
            ...
            <ul className="footer__links">
              ...
              <li><Link to={pathFor('/#apps-mobile')}>{t(uiCopy.anchors.apps)}</Link></li>
              <li><Link to={pathFor('/#modelos')}>{t(uiCopy.anchors.templates)}</Link></li>
        ...
        <div className="footer__bottom">
          ...
          <span data-empresa-legal>{legal}</span>
        </div>
      </div>
    </footer>
  )
